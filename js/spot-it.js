function getNumberOfSymbols(numExpectedCards) {
    switch (numExpectedCards) {
        case 13:
            return 4;
        case 31:
            return 6;
...
function onGenerate() {
    dobble(parseInt(document.getElementById('cards-selector').value, 10));
}

function generateSymbolValues(numberOfSymbols) {
    var result = [];
    for (var i = 0; i < numberOfSymbols; i++) {
        result.push(generateAdditionValues(result));
    }
    return result;
}

function isInAdditionArray(number, valuesArray) {
    return valuesArray.some((num) => num === number);
}
...
function getAdditionOptions(number) {

    const sum1 = getRandomInteger(number - 1);
    const sum2 = number - sum1;
    return `${sum1} + ${sum2}`;

}

function generateAdditionValues(allValues) {
    const results = [];
    var newValue = 0;
    do {
      newValue = getRandomInteger(50);
    } while (isInAdditionArray(newValue, allValues))
    const option = getAdditionOptions(newValue);
    results.push(newValue, option);
    return results;
}

function getValue(value) {
    const randomIndex = getRandomInteger(2);
    return symbolValues[value][randomIndex];
}

function dobble(numExpectedCards) {
    var N = getNumberOfSymbols(numExpectedCards);     // number of symbols on each car
    symbolValues = generateSymbolValues(numExpectedCards);
